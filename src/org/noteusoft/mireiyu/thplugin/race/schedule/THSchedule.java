...
{
    public void run1(final THPlugin plugin0, final String thrpre0)
    {
        new BukkitRunnable() {
            public void run() {
                for (final Player player : Bukkit.getOnlinePlayers()){
                    ...
                    if (player.hasPermission("thr.skill")){
                        ...
                        if ((plugin0.getConfig().getString("user." + player.getUniqueId() + ".race").toString().contains("youma")) || (plugin0.getConfig().getString("user." + player.getUniqueId() + ".race").toString().contains("kappa")) || (plugin0.getConfig().getString("user." + player.getUniqueId() + ".race").toString().contains("tenngu"))){
                            ...
                            if ((!player.isDead()) && (plugin0.getConfig().getDouble("user." + player.getUniqueId() + ".spilit") >= 20.0D) && (((MetadataValue)player.getMetadata("spilituse").get(0)).asDouble() > 0.0D)){
    ...
    public void run2(final THPlugin plugin0, final String thrpre0)
    {
        new BukkitRunnable() {
            public void run() {
    ...
    public void run3(final THPlugin plugin0, String thrpre0)
    {
        new BukkitRunnable() {
            public void run() {
